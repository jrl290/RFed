...
messaging with offline delivery, cross-node synchronisation, mobile push
...
| `<opaque-secret>` | Private; distributed out-of-band | `s3cr3t-invite-token` |
...
For **private** channels the name is an unguessable secret — possession of
the name equals channel membership.
...
- **inner_blob**: Opaque ciphertext (encrypted to the channel's X25519 key,
  signed by the sender). Immutable — never modified by the node.
- **stamp**: Proof-of-work stamp appended by the sender. Validated and
  stripped on ingest; only the clean blob is stored and synced.
...
blob arrives for an offline subscriber. This enables mobile push
notifications without the node holding platform credentials.
...
- The relay is responsible for converting the wake signal into a platform
